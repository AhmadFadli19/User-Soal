...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <div class="flex items-center mb-4">
            ...
            <h1 class="text-3xl font-bold text-gray-800">Edit Course Content</h1>
        </div>
    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.courses.contents.update', [$course, $content->type, $content->id]) }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select id="type" name="type" onchange="toggleQuestionFields()" 
                    ...
                    <option value="story" {{ old('type', $content->type) === 'story' ? 'selected' : '' }}>Story (Explanation/Reading Material)</option>
                    <option value="question" {{ old('type', $content->type) === 'question' ? 'selected' : '' }}>Question (Multiple Choice Quiz)</option>
                </select>
            ...
            <div id="question-fields" class="{{ $content->type === 'question' ? '' : 'hidden' }}">
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="correct_answer" name="correct_answer" value="{{ old('correct_answer', $content->correct_answer) }}" 
...
<script>
    ...
    if (type === 'question') {
        ...
        // Remove required attribute for story type
        const correctAnswerField = document.getElementById('correct_answer');
        if (correctAnswerField) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // FIXED: Remove the problematic submit event listener
    // Instead of disabling fields, remove their name attribute so they won't be submitted
    document.querySelector('form').addEventListener('submit', function(e) {
        const type = document.getElementById('type').value;
        
        if (type === 'story') {
            // Remove name attribute instead of disabling to prevent submission
            const correctAnswerField = document.getElementById('correct_answer');
            if (correctAnswerField) {
                correctAnswerField.removeAttribute('name');
            }
            document.querySelectorAll('input[name="options[]"]').forEach(input => {
